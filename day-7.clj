...
(defn base-tower
	[tower-defs]
	(first (set/difference (all-towers tower-defs)
	                       (all-subtowers tower-defs))))
...
(defn aggregate-weight
  ([tower-defs]
   (aggregate-weight (bottom-up-tree tower-defs) (base-tower tower-defs)))
...
(defn unbalanced
	...
	(let [freqs (frequencies subtower-weights)
	      diff-weight (reverse-lookup freqs 1)
	      subtowers-to-weights (map vector subtowers subtower-weights)]
    (reverse-lookup subtowers-to-weights diff-weight)))
